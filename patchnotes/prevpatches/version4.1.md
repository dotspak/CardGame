# Pecto V4.1 Patch Notes

---

## Overview

With the Pecto Simulator on to the horizon, its time for a small update to the core of the game. No cards are being updated this patch. The main point of this patch is to address some small issues to make the game slightly slower (or at least give the potential for a slightly slower game). A few of these changes are due to the upcoming simulator's limitations that have bred new ideas.

*This will be probably be the last update until the simulator. The simulator update will still be called 4.2.*
...
- Quick now only applies when attacking, not blocking. This means that a unit with Quick and Bleed is no longer unkillable in combat (at least on defense).
...
- A new interaction trigger is now being added to the defender's turn.
...
When two units swap, only the unit who actually declared the move becomes inactive.
...
- *Note: at the time of this update not all passives have been converted, but will be in the simulator.*
