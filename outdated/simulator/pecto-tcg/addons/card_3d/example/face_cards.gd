...
func _generate_all_face_cards() -> Dictionary:
	...
	for suit in Suit:
		for rank in Rank:
			var front_material = "res://addons/card_3d/example/materials/" + str(suit).to_lower() + "-" + str(Rank[rank]) + ".tres"
			var back_material = "res://addons/card_3d/example/materials/card-back.tres"
			var card_data = {
			"rank": Rank[rank],
			"suit": Suit[suit],
